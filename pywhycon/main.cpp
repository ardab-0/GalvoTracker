...
void mouse_callback(int event, int x, int y, int flags, void* param) {
  if (event == cv::EVENT_LBUTTONDOWN) { cout << "clicked window" << endl; clicked = true; }
}
...
int main(int argc, char** argv)
{
  ...
  if (is_camera) {
    ...
    if (config_vars.count("width")) {
      capture.set(cv::CAP_PROP_FRAME_WIDTH, config_vars["width"].as<int>());
      capture.set(cv::CAP_PROP_FRAME_HEIGHT, config_vars["height"].as<int>());
    }
  }
  else {
  ...
  if (do_tracking) {
    ...
    video_writer.open(output_name + ".avi", cv::VideoWriter::fourcc('M','J','P','G'), 15, frame_size);
