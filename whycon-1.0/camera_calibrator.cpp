...
void mouse_callback(int event, int x, int y, int flags, void* param) {
  if (event == cv::EVENT_LBUTTONDOWN) clicked = true;
  if (event == cv::EVENT_RBUTTONDOWN) rclicked = true;
}
...
int main(int argc, char** argv) {
  ...
  capture1.set(cv::CAP_PROP_FRAME_WIDTH, width);
  capture1.set(cv::CAP_PROP_FRAME_HEIGHT, height);
  capture1.set(cv::CAP_PROP_FPS, 20);
  ...
  if (argc == 9) {
    ...
    capture2.set(cv::CAP_PROP_FRAME_WIDTH, width);
    capture2.set(cv::CAP_PROP_FRAME_HEIGHT, height);
    capture2.set(cv::CAP_PROP_FPS, 20);
  ...
  while (true) {    
    ...
    int result = cv::findChessboardCorners(frame1, pattern_size, corners, cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_NORMALIZE_IMAGE);
    if (result) {
      cv::Mat gray;
      cv::cvtColor(frame1, gray, cv::COLOR_BGR2GRAY);
      cv::cornerSubPix(gray, corners, cv::Size(11, 11), cv::Size(-1, -1), cv::TermCriteria(cv::TermCriteria::EPS + cv::TermCriteria::MAX_ITER, 100, 0.05));
    ...
    cv::putText(frame1, ostr.str(), cv::Point(5, 15), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255,0,255), 1.5, cv::LINE_AA);
